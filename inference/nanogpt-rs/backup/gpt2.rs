...
impl CausalSelfAttention {
    ...
    fn forward(&self, xs: &Tensor, cache: &mut Option<(Tensor, Tensor)>) -> candle_core::Result<Tensor> {
        ...
        *cache = Some((k.clone(), v.clone()));
        ...
        // OPTIMIZATION: Only apply causal mask during prefill (seq_len > 1)
        let att = if seq_len > 1 {
            ...
            att // Skip the mask entirely! GPU goes brrrr.
        };
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    // Try to grab the first Apple Silicon GPU (index 0)
    let device = Device::new_metal(0).unwrap_or_else(|_| {
        println!("Metal GPU not found, falling back to CPU.");
        Device::Cpu
    });
    ...
    // If the user provided arguments, join them into a single prompt
    // Otherwise, fall back to a default prompt
    let prompt = if args.len() > 1 {
        args[1..].join(" ")
    } else {
        "Why did the AI cross the road?".to_string()
    };

    let mut tokens = tokenizer.encode(prompt.clone(), false)?.get_ids().to_vec();
    print!("\nPrompt: {}\nOutput: ", prompt);
    std::io::stdout().flush()?;
    ...
    for _ in 0..500 {
        ...
        // Break early if the model naturally ends its thought
        if next_token == 50256 {
            break;
        }
    }
    ...
    println!("\n\n--- Rust Performance ---");
    println!("Generated {} tokens in {:.4} seconds", tokens_generated, elapsed);
    println!("Speed: {:.2} TPS", tps);
